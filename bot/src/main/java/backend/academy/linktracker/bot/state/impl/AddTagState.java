...
public class AddTagState extends AbstractState {
    ...
    public void onEnter(Long chatId) {
        ...
        telegramBotService.sendMessage(chatId, "Введите ссылку, к которой вы хотите добавить тег");
        userStateService.updateUserSubstate(chatId, UserState.SubState.WAITING_FOR_URL);
    }

    @Override
    public UserState getState() {
        return UserState.ADD_TAG;
    }
    ...
    private void processUrl(Update update) {
        ...
        if (uriHandler.canHandle(update)) {
            ...
            telegramBotService.sendMessage(chatId, "Введите желаемый тег ");
        } else {
            telegramBotService.sendMessage(
                    chatId,
                    "Некорретная ссылка. Введите ссылку еще раз или введите /cancel, чтобы отменить процесс добавления тега");
        }
    }
        ...
        if (tagsHandler.canHandle(update)) {
            ...
            telegramBotService.sendMessage(
                    ...
                    "Некорретный тег." + " Введите тег еще раз или введите /cancel,"
                            + " чтобы отменить процесс добавления тега.");
        }
    }
    ...
    private void addTag(Update update) {
        ...
        scrapperGrpcService.addTag(
                chatId, link.getLink().toString(), link.getTags().getFirst());
        telegramBotService.sendMessage(chatId, "Тег добавлен");
