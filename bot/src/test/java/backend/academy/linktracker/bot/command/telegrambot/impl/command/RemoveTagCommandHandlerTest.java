...
class RemoveTagCommandHandlerTest {
    ...
    @InjectMocks
    private RemoveTagCommandHandler removeTagCommandHandler;

    @Test
    void shouldSwitchStateToRemoveTagAndInitializeSession() {
        when(update.message()).thenReturn(message);
        when(message.chat()).thenReturn(chat);
        when(chat.id()).thenReturn(11L);

        removeTagCommandHandler.processCommand(update);

        verify(linkSessionService).updateSession(any());
        verify(userStateService).updateUserState(11L, UserState.REMOVE_TAG);
        verify(userStateService).updateUserSubstate(11L, UserState.SubState.START);
    }
}
