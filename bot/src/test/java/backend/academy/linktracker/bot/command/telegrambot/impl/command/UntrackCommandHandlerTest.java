...
class UntrackCommandHandlerTest {
    ...
    @Test
    void shouldSwitchStateToUntrackingAndInitializeSession() {
        when(update.message()).thenReturn(message);
        when(message.chat()).thenReturn(chat);
        when(chat.id()).thenReturn(100L);

        untrackCommandHandler.processCommand(update);

        verify(linkSessionService).updateSession(any());
        verify(userStateService).updateUserState(100L, UserState.UNTRACKING);
    }
}
